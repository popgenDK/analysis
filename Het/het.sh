...
#Run angsd with the option -dosaf 1 to generate the .saf.idx file that will be used by winSFS to estimate the REF/REF, ALT/REF, ALT/ALT sites.
...
## collect all estimates into one file and modify the sample names to drop the _est.ml suffix
awk '{print FILENAME, $0}' *_est.ml | grep -v "#" | sed 's/all_est.ml //g' | sed 's/_est.ml//g' > all_est.ml

# load R module and run the script to generate the heterozygosity plot
